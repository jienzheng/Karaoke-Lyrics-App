...
from app.models.schemas import GuestJoinRequest, GuestJoinResponse, SpotifyToken, User
...
async def login():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate auth URL: {e!s}")


@router.get("/callback")
async def callback(code: str | None = Query(None), error: str | None = Query(None)):
    """
...
async def get_current_user(request: Request):
    """
    Get current user profile. Reads token from Authorization: Bearer header.
    """
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail=f"Invalid access token: {e!s}")
...
async def guest_join(request: GuestJoinRequest):
    ...
    session_result = (
        db.table("sessions")
        .select("*")
        .eq("code", request.session_code.upper())
        .eq("is_active", True)
        .execute()
    )
